...
def sum_array(array)
    # return array.to_i if array.length == 1
    return 0 if array.length == 0

    #   (sum_array(array[0..-2]) + sum_array(array[-1]))
    # (sum_array(array[0..(-2)]) + sum_array(array[(-1)])).sum
    # sum_array(array - array[-1]).sum
    # sum_array(array).sum - sum_array(array[-1]).sum
...
def flatten(data)    
    # data.select {|arr| arr if arr.length < 2} + flatten(data.select {|arr| arr if arr.length > 1})
